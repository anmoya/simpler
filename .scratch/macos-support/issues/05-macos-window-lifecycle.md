...
**Update**: the initial `RunEvent::Reopen` handling compiled fine on this Mac but broke the Linux build — `Reopen` is `#[cfg(target_os = "macos")]`-gated inside the `tauri` crate itself (not just semantically macOS-only), so matching on it unconditionally in `App::run`'s callback is a compile error on Linux. Caught by ticket 07's CI on the first real tagged push (`v0.1.7`, which never got as far as publishing). Fixed by gating the whole `app.run(...)` closure to macOS, with a no-op callback (`app.run(|_, _| {});`) on other platforms (commit `7f4691a`); both platforms confirmed green on `main` afterward, and `v0.1.8` shipped clean. See ticket 08's comments for the full release-side story.
