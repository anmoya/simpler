...
**Not independently verified**: an actual tagged release running through GitHub Actions end-to-end (both build jobs succeeding, the publish job's merge, a real signed `.dmg`/`.app.tar.gz` landing on a GitHub Release). That requires pushing a `v*.*.*` tag, which this agent is treating as an action needing the user's go-ahead rather than something to trigger unattended.
